...
def TraceQueryBatch3P(data, host="localhost", port=8002):
    ...
    for i in range(len(data) / 9):
        data_string = data_string + str(data[i * 9 + 0]) + "," + str(data[i * 9 + 1]) + "," + str(data[i * 9 + 2]) + "," + str(
...
class RoadGraph:
    ...
    def findNearbySegments(self, score_threshold=1.0, limitation=100000000, region=None, dist_threshold=0.00035):
        ...
        for edgeid in self.edges.keys():
            ...
            if edgeid in self.edgeScore.keys():
                ...
                dir_lat, dir_lon = latlonNorm((self.nodes[node2][0] - self.nodes[node1][0], self.nodes[node2][1] - self.nodes[node1][1]),
                                              lat=self.nodes[node2][0])
                norm_lat = -dir_lon
                norm_lon = dir_lat
                ...
                for id_ in possibleEdges:
                    ...
                    can_dir_lat, can_dir_lon = latlonNorm(
                        (self.nodes[can_node2][0] - self.nodes[can_node1][0], self.nodes[can_node2][1] - self.nodes[can_node1][1]),
                        lat=self.nodes[node2][0])
            ...
            for p_edgeId in possibleEdges:
                ...
                if (d1 > 0 and d2 > 0 and d3 > 0 and d4 > 0) or (d1 < 0 and d2 < 0 and d3 < 0 and d4 < 0):
                    if (edgeId, p_edgeId) not in edgePairsMask and (p_edgeId, edgeId) not in edgePairsMask:
                        ...
                        center_lat1, center_lon1, _, _ = edgeIntersection(lat1, lon1, dlat1, dlon1 / math.cos(math.radians(lat1)), lat3,
                                                                          lon3, lat4, lon4)
    ...
    def Dump2GeoJson(self, filename, region=[-200, -200, 200, 200]):
        ...
        for edgeId, edge in self.edges.iteritems():
            ...
            if inside == True:
                myfeature.append(
                    Feature(properties={"id": edgeId, "type": "residential"}, geometry=LineString([(lon1, lat1), (lon2, lat2)])))
...
class RoadGraphCombiner:
    def __init__(self, RoadGraphA, RoadGraphB, ConnectivityChecker=None, CNNOutput=None, region=None):
                ...
                for possibleEdge in possibleEdges:
                    ...
                    if dist < min_dist and ok == 1:
                        ...
                        if self.CNNConnectivityChecker(lat, lon, newlat, newlon, cnn_dat, region) == True:
                            if self.ConnectivityChecker(avglat, avglon, pn1lat, pn1lon, pn0lat, pn0lon) == True or self.ConnectivityChecker(
                                    avglat, avglon, pn2lat, pn2lon, pn3lat, pn3lon) == True:
                                RoadGraphA.addEdgeToOneExistedNode("Tmp" + str(newNodeId), newlat, newlon, old_n1, reverse=True,
                                                                   nodeScore1=100, edgeScore=100)
                                RoadGraphA.addEdgeToOneExistedNode("Tmp" + str(newNodeId), newlat, newlon, old_n2, reverse=True,
                                                                   nodeScore1=100, edgeScore=100)

                                RoadGraphA.addEdge("Tmp" + str(newNodeId), newlat, newlon, "CNN" + str(n1), lat, lon, reverse=True,
                                                   nodeScore1=100, nodeScore2=100, edgeScore=100)
                    ...
                    if dist < min_dist and ok == 1:
                        ...
                        if self.CNNConnectivityChecker(lat, lon, newlat, newlon, cnn_dat, region) == True:
                            if self.ConnectivityChecker(avglat, avglon, pn1lat, pn1lon, pn0lat, pn0lon) == True or self.ConnectivityChecker(
                                ...
                                RoadGraphA.addEdge("Tmp" + str(newNodeId), newlat, newlon, "CNN" + str(n2), lat, lon, reverse=True,
                                                   nodeScore1=100, nodeScore2=100, edgeScore=100)
        ...
        for edgeId, edge in RoadGraphB.edges.iteritems():
            ...
            RoadGraphA.addEdge("CNN" + str(n1), lat1, lon1, "CNN" + str(n2), lat2, lon2, reverse=True, nodeScore1=100, nodeScore2=100,
                               edgeScore=100)
...
def CheckParallelSegments(s1, s2, lat_res, lon_res, image_size=512, folder="", start_lat=0, start_lon=0, resolution=1024, padding=128,
                          zoom=18, scale=1, output_id=0):
    ...
    m, ok = md.GetMapInRect(range_minlat, range_minlon, range_maxlat, range_maxlon, folder=folder, start_lat=start_lat, start_lon=start_lon,
                            resolution=resolution, padding=padding, zoom=zoom, scale=scale)
...
if __name__ == "__main__":
    data = [42.355538, -71.083505, 0.00030, float(sys.argv[1]), 42.352176, -71.081198, 0.00030, float(sys.argv[1]), 42.352176, -71.081198,
            0.00030, float(sys.argv[1])]
