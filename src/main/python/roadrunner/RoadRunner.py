...
def TraceQueryBatch3P(data, host="localhost", port=8002):
    ...
    for i in range(len(data) / 11):
        data_string = data_string + str(data[i * 11 + 0]) + "," + str(data[i * 11 + 1]) + "," + str(data[i * 11 + 2]) + "," + str(
            data[i * 11 + 3]) + "," + str(data[i * 11 + 4]) + "," + str(data[i * 11 + 5]) + "," + str(data[i * 11 + 6]) + "," + str(
...
class RoadTree:
    ...
    def node3ScoreWithRange(self, nodeA, nodeB, nodeC, r1=QueryRange, r2=QueryRange, r3=QueryRange, d1=-1, d2=-1, stepSize=StepSize,
                            DegreeResolution=DegreeResolution, center=0, size=16):  # A->B->C
    ...
    def node3ScoreHorizontal(self, nodeA, nodeB, nodeC, r1=QueryRange, r2=QueryRange, r3=QueryRange, d1=-1, d2=-1, stepSize=0.00003,
                             DegreeResolution=DegreeResolution, degree=0, size=16):  # A->B->C
        score = [1] * (size * 2 + 1)
        data = []
        ...
        for i in range(-size, size + 1):
            ...
            data.append(
                nodeC['lon'] + math.cos(math.radians(degree * DegreeResolution)) * stepSize * i / math.cos(math.radians(nodeB['lat'])))
            data.append(r1)
            data.append(r2)
            data.append(r3)
            ...
            nodeC['rawScoreHorizontalLoc'].append((nodeC['lat'] + math.sin(math.radians(degree * DegreeResolution)) * stepSize * i,
                                                   nodeC['lon'] + math.cos(
                                                       math.radians(degree * DegreeResolution)) * stepSize * i / math.cos(
                                                       math.radians(nodeB['lat']))))
    ...
    def node3ScoreHorizontalVStyle(self, nodeA, nodeB, nodeC, r1=QueryRange, r2=QueryRange, r3=QueryRange, d1=-1, d2=-1, stepSize=0.00003,
                                   DegreeResolution=DegreeResolution, degree1=0, degree2=0, size=16):  # A->B->C
        score = [1] * (size * 2 + 1)
        data = []
        ...
        for i in range(size, 0, -1):
            ...
            data.append(
                nodeC['lon'] + math.cos(math.radians(degree1 * DegreeResolution)) * stepSize * i / math.cos(math.radians(nodeB['lat'])))
            data.append(r1)
            data.append(r2)
            data.append(r3)
            ...
            nodeC['rawScoreHorizontalLoc'].append((nodeC['lat'] + math.sin(math.radians(degree1 * DegreeResolution)) * stepSize * i,
                                                   nodeC['lon'] + math.cos(
                                                       math.radians(degree1 * DegreeResolution)) * stepSize * i / math.cos(
                                                       math.radians(nodeB['lat']))))
        ...
        for i in range(size + 1):
            ...
            data.append(
                nodeC['lon'] + math.cos(math.radians(degree2 * DegreeResolution)) * stepSize * i / math.cos(math.radians(nodeB['lat'])))
            data.append(r1)
            data.append(r2)
            data.append(r3)
            ...
            nodeC['rawScoreHorizontalLoc'].append((nodeC['lat'] + math.sin(math.radians(degree2 * DegreeResolution)) * stepSize * i,
                                                   nodeC['lon'] + math.cos(
                                                       math.radians(degree2 * DegreeResolution)) * stepSize * i / math.cos(
                                                       math.radians(nodeB['lat']))))
    ...
    def updateScore(self, nodeID):
        ...
        new_score = self.node3Score(self.nodes[node_list[n3]], self.nodes[node_list[n2]], self.nodes[node_list[n1]], r1=0.00005, r2=0.00005,
                                    r3=0.00003)
        ...
        for i in range(4):
            ...
            new_score = self.node3Score(self.nodes[node_list[n3]], self.nodes[node_list[n2]], self.nodes[node_list[n1]], r1=r, r2=r,
                                        r3=0.00005)
            ...
            if self.nodes[nodeID]['rawScore'][j] > 0:
                ...
                for i in range(kernel_size / 2):
                    if tmp_score[(j + i + 1 + 360 / DegreeResolution) % (360 / DegreeResolution)] > 0:
                        s = s + tmp_score[(j + i + 1 + 360 / DegreeResolution) % (360 / DegreeResolution)] * gaussian_kernel[
                            i + kernel_size / 2 + 1]
                    else:
                        break
    ...
    def numberOfPeaks(self, nodeID, itemname='rawScore', center=0, r=24):
        ...
        for i in range(-r + 1, r - 1):
            ...
            if self.nodes[nodeID][itemname][ind] > self.nodes[nodeID][itemname][indp] and self.nodes[nodeID][itemname][indp] > 0 and \
                    self.nodes[nodeID][itemname][ind] > self.nodes[nodeID][itemname][indn] and self.nodes[nodeID][itemname][indn] > 0:
                n = n + 1
    ...
    def getPeaks(self, nodeID, itemname='rawScore'):
        ...
        for i in range(1, n - 1):
            if self.nodes[nodeID][itemname][i] > self.nodes[nodeID][itemname][i + 1] and self.nodes[nodeID][itemname][i + 1] > 0 and \
                    self.nodes[nodeID][itemname][i] > self.nodes[nodeID][itemname][i - 1] and self.nodes[nodeID][itemname][i - 1] > 0:
                peaks.append(i)
        ...
        for path in paths:
            ...
            if isLocalPath == True:
                ...
                for i in range(3):
                    ...
                    if i == 0:
                        new_score1 = self.node3ScoreHorizontalVStyle(self.nodes[node_list[n3][1]], self.nodes[node_list[n2][1]],
                                                                     self.nodes[node_list[n1][1]], r1=r, r2=r, r3=0.00002,
                                                                     d1=dist_list[n3] - dist_list[n2], d2=dist_list[n2] - dist_list[n1],
                                                                     stepSize=0.000015, DegreeResolution=5, degree1=(direction + 24) % 72,
                                                                     degree2=(direction - 24 + 72) % 72, size=DBDCheckRange)

                    elif i == 1:
                        new_score1 = self.node3ScoreHorizontalVStyle(self.nodes[node_list[n4][1]], self.nodes[node_list[n3][1]],
                                                                     self.nodes[node_list[n1][1]], r1=r, r2=r, r3=0.00002,
                                                                     d1=dist_list[n4] - dist_list[n3], d2=dist_list[n3] - dist_list[n1],
                                                                     stepSize=0.000015, DegreeResolution=5, degree1=(direction + 24) % 72,
                                                                     degree2=(direction - 24 + 72) % 72, size=DBDCheckRange)

                    elif i == 2:
                        new_score1 = self.node3ScoreHorizontalVStyle(self.nodes[node_list[n5][1]], self.nodes[node_list[n3][1]],
                                                                     self.nodes[node_list[n1][1]], r1=r, r2=r, r3=0.00002,
                                                                     d1=dist_list[n5] - dist_list[n3], d2=dist_list[n3] - dist_list[n1],
                                                                     stepSize=0.000015, DegreeResolution=5, degree1=(direction + 24) % 72,
                                                                     degree2=(direction - 24 + 72) % 72, size=DBDCheckRange)
                    ...
                    self.nodes[nodeID]['deferredLinksInfo'] = [node_list[n2][1], node_list[n3][1], r, r, node_list[n4][1],
                                                               dist_list[n3] - dist_list[n2], dist_list[n2] - dist_list[n1]]
    ...
    def updateNewNode(self, new_id, Forest, _parent, parent_lat, parent_lon, _direction, max_score):
        ...
        if self.config['keep_deadend'] == True:
            ...
            if v1 == 0 and v2 == 0 and v3 == 0:
                ...
                if len(ns) == 0:
                    ...
                    if self.nodes[new_id]['Aux'] == 1 or self.nodes[self.nodes[new_id]['parent']]['Aux'] == 1 or \
                            self.nodes[self.nodes[self.nodes[new_id]['parent']]['parent']]['Aux'] == 1:
                        pass
                    else:
                        self.nodes[new_id]['OutRegion'] = 1
        ...
        if v1 == 0 and v2 == 0 and v3 == 0:
            ...
            if abs(new_lat - self.Region[0]) < 0.00025 or abs(new_lat - self.Region[2]) < 0.00025 or abs(
                    new_lon - self.Region[1]) < 0.00040 or abs(new_lon - self.Region[3]) < 0.00040:
                self.nodes[new_id]['OutRegion'] = 1
        ...
        possible_links = list(self.index_deferredLinks.intersection((min(self.nodes[new_id]['lat'], parent_lat),
                                                                     min(self.nodes[new_id]['lon'], parent_lon),
        ...
        for peak in peaks:
            ...
            if peak < DBDCheckRange - DBDExclusionRange or peak > DBDCheckRange + DBDExclusionRange:
                ...
                possible_segment = list(
                    self.index_segment.intersection((peaklat - r_lat, peaklon - r_lon, peaklat + r_lat, peaklon + r_lon)))
                        ...
                        if ddd / 32.0 < 0.00020:
                            ...
                            while True:
                                if cur_node['referenceCounter'] == 0 and cur_node['OutRegion'] != 1 and cur_node['removed'] == False:
                                    ...
                                    if abs(new_lat - self.Region[0]) < 0.00025 * 2 or abs(new_lat - self.Region[2]) < 0.00025 * 2 or abs(
                                            new_lon - self.Region[1]) < 0.00040 * 2 or abs(new_lon - self.Region[3]) < 0.00040 * 2:
                                        break
    ...
    def explore(self, Reigon=None, Forest=None, RegionImage=None, mCNNOutput=None, exitMask=None, exitPoint=None, BFS=True):
        ...
        def NextNodes(raw_score, cnn_score, mask):
            ...
            for i in range(n):
                ...
                if raw_score[i] > 0 and raw_score[(i + 1) % n] > 0 and raw_score[(i - 1 + n) % n] > 0 and raw_score[i] >= raw_score[
                    (i - 1)] and raw_score[i] >= raw_score[(i + 1) % n]:
        ...
        if _parent > -1 and max_score > 0:
            ...
            # Half step size
            node = InitNode(parent_lat + math.sin(math.radians(_direction * DegreeResolution)) * StepSize / 2,
                            parent_lon + math.cos(math.radians(_direction * DegreeResolution)) / math.cos(
                                math.radians(parent_lat)) * StepSize / 2, new_id, new_parent, [], _direction)
            self.nodes.append(node)
    ...
    def explore(self, idx=0):
        ...
        for tree in self.RoadTrees:
            ...
            if self.oks[tcounter] == True:
                ...
                self.oks[tcounter], score = tree.explore(self.Region, self.RoadTrees, self.RegionImage, self.CNNOutput, self.ExitMask,
                                                         self.ExitPoints)
...
if __name__ == "__main__":
    ...
    dumpname += "_" + str(config['minimal_number_of_trips']) + "_" + str(config['minimal_number_of_trips_deferred_branch']) + "_" + str(
        config['history_length']) + "_" + str(config['keep_deadend'])
    ...
    for i in xrange(500000):
        ...
        if roadForest.exploreOneByOne(idx=i) == False:
            roadForest.dump(config['OutputFolder'] + "/output_file_last")
