...
def destination_point(a_lat, a_lon, bearing, distance):
    ...
    b_lat = math.asin(
        math.sin(a_lat) * math.cos(angular_distance) + math.cos(a_lat) * math.sin(angular_distance) * math.cos(bearing))
...
def intersection_point(a_lat, a_lon, a_bearing, b_lat, b_lon, b_bearing, debug=False):
    ...
    dist12 = 2.0 * math.asin(math.sqrt(
        math.sin(dLat / 2.0) * math.sin(dLat / 2.0) + math.cos(a_lat) * math.cos(b_lat) * math.sin(
    ...
    alpha3 = math.acos(
        -1.0 * math.cos(alpha1) * math.cos(alpha2) + math.sin(alpha1) * math.sin(alpha2) * math.cos(dist12))
    dist13 = math.atan2(math.sin(dist12) * math.sin(alpha1) * math.sin(alpha2),
                        math.cos(alpha2) + math.cos(alpha1) * math.cos(alpha3))
    c_lat = math.asin(math.sin(a_lat) * math.cos(dist13) + math.cos(a_lat) * math.sin(dist13) * math.cos(brng13))
    dLon13 = math.atan2(math.sin(brng13) * math.sin(dist13) * math.cos(a_lat),
