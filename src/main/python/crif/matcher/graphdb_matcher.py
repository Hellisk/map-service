...
class GraphDBMatcher(GPSMatcher):
    ...
    def mapdb_to_hmm(self, mapdb):
        ...
        for from_edge in themap.edges.values():
            ...
            for to_edge in from_edge.out_edges:
                ...
                if (to_edge.out_node != from_edge.in_node):
                    hmm[from_edge_key] += [
                        (to_edge_key, (to_edge.weight / to_edges_sum_weight) * (1.0 - TRANSITION_SELF - TRANSITION_UNKNOWN))]
        ...
        hmm['unknown'] += [((edge.in_node.coords(), edge.out_node.coords()), (1 - TRANSITION_UNKNOWN_UNKNOWN) / len(themap.edges)) for edge
                           in themap.edges.values()]
