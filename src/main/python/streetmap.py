...
class Edge:
    ...
    @property
    def bearing(self):
        return spatialfunclib.path_bearing(self.in_node.latitude, self.in_node.longitude, self.out_node.latitude,
                                           self.out_node.longitude)

    def point_at_meters_along(self, meters):
        return spatialfunclib.point_along_line(self.in_node.latitude, self.in_node.longitude, self.out_node.latitude,
                                               self.out_node.longitude, meters / self.length)
...
class StreetMap:
    ...
    def load_graphdb(self, grapdb_filename):
        ...
        for id, edge_ids in query_result:
            ...
            self.segment_lookup_table[(self.segments[id].head_edge.in_node, self.segments[id].tail_edge.out_node)] = \
                self.segments[id]
    ...
    def _index_edges(self):
        ...
        for curr_edge in self.edges.values():
            ...
            self.edge_spatial_index.insert(curr_edge.id,
                                           (curr_edge_minx, curr_edge_miny, curr_edge_maxx, curr_edge_maxy))
    ...
    def _find_and_index_intersections(self):
        ...
        for intersection_node in intersection_nodes:
            ...
            if (intersection_node not in placed_intersection_nodes):
                # create bounding box
                bounding_box = (intersection_node.longitude - lon_offset, intersection_node.latitude - lat_offset,
                                intersection_node.longitude + lon_offset, intersection_node.latitude + lat_offset)
