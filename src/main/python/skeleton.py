...
class GrayscaleSkeleton:
    ...
    def thin_pixels(self, image):
        ...
        neighbors = nd.convolve((image > 0).astype(np.int), [[1, 1, 1], [1, 0, 1], [1, 1, 1]], mode='constant',
                                cval=0.0)
    ...
    def PRE_first_subiteration(self, curr_image, fg_pixels):
        ...
        for (i, j) in fg_pixels:
            ...
            if (2 <= (bool(p2) + bool(p3) + bool(p4) + bool(p5) + bool(p6) + bool(p7) + bool(p8) + bool(p9)) <= 6 and
                    (p2 * p4 * p6 == 0) and
                    (p4 * p6 * p8 == 0)):
                if ((not p2 and p3) + (not p3 and p4) + (not p4 and p5) + (not p5 and p6) + (not p6 and p7) + (
                        not p7 and p8) + (not p8 and p9) + (not p9 and p2) == 1):
                    zero_pixels[(i, j)] = 0
                    if p2 == 1:
                        next_pixels[(i - 1, j)] = 0
                    if p3 == 1:
