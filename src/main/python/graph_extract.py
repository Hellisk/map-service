...
def douglas_peucker(segment, epsilon):
    ...
    for i in range(1, len(segment) - 1):
        (_, _, d) = projection_onto_line(segment[0].latitude, segment[0].longitude, segment[-1].latitude,
                                         segment[-1].longitude, segment[i].latitude, segment[i].longitude)
...
class Graph:
    ...
    def create_graph(self, main_crossings, segments, density_estimate, output_filename):
        nodes, new_segments, intersections = self.create_nodes_and_new_segments(main_crossings, segments,
                                                                                density_estimate)
        ...
        for segment in new_segments:
            ...
            for i in range(0, len(smoothed_segment) - 1):
                cur.execute("INSERT INTO edges VALUES (" + str(edge_id) + "," + str(smoothed_segment[i].id) + "," + str(
                    smoothed_segment[i + 1].id) + "," + str(segment_weight) + ")")
            ...
            for i in range(0, len(smoothed_segment) - 1):
                cur.execute(
                    "INSERT INTO edges VALUES (" + str(edge_id) + "," + str(smoothed_segment[i + 1].id) + "," + str(
                        smoothed_segment[i].id) + "," + str(segment_weight) + ")")
            ...
            cur.execute(
                "INSERT INTO segments VALUES (" + str(segment_id) + ",'" + str(outbound_segment_edge_ids) + "')")
            segment_id += 1
    ...
    def create_nodes_and_new_segments(self, main_crossings, segments, density_estimate):
        ...
        for segment in segments:
            ...
            if (head_node not in nodes):
                nodes[head_node] = Node(pixels_to_coords(head_node),
                                        density_map[density_estimate[segment[0][0], segment[0][1]] - 1])
            ...
            for i in range(1, len(segment) - 1):
                if (segment[i] not in nodes):
                    nodes[segment[i]] = Node(pixels_to_coords(segment[i]),
                                             density_map[density_estimate[segment[i][0], segment[i][1]] - 1])
            ...
            if (tail_node not in nodes):
                nodes[tail_node] = Node(pixels_to_coords(tail_node),
                                        density_map[density_estimate[segment[-1][0], segment[-1][1]] - 1])
    ...
    def identify_crossing_points(self, skeleton):
        ...
        for (i, j) in izip(fg_pixels[0], fg_pixels[1]):
            ...
            fringe = [bool(p[8] and bool(p[7] or p[0] or p[1])), bool(p[9] and bool(p[0] or p[1])),
