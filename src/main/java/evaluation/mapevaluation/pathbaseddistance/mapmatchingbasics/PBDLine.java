...
Filename: LineTM.java
 */
package evaluation.mapevaluation.pathbaseddistance.mapmatchingbasics;

public class PBDLine {
	double xdiff, ydiff;
	double c, m, theta;
	private PBDVertex p1;
	private PBDVertex p2;
	
	public PBDLine(PBDVertex p1, PBDVertex p2) {
		this.p1 = p1;
		this.p2 = p2;
	...
	public double[] pIntersection(PBDVertex p, double eps, boolean precision) {
		
		double[] t = new double[2];
		/*
		 * LineTM 1: x1+(x2-x1)*t = x; x1+xdiff*t = x y1+(y2-y1)*t = y; y1+ydiff*t
		 * = y
		 *
		...
		double new_c_t = (p1.x - p.x) * (p1.x - p.x) + (p1.y - p.y) * (p1.y - p.y) - newEps * newEps;
	...
	public boolean pIntersection(PBDEdge e, double eps) {
		
		/*
		 * LineTM 1: x1+(x2-x1)*t = x y1+(y2-y1)*t = y LineTM 2: y = mx + c
		 * y1+(y2-y1)*t = (x1+(x2-x1)*t)*m + c (y2-y1)*t - (x2-x1)*t*m = x1*m +
		 * c- y1 t = (x1*m + c - y1)/((y2-y1)-(x2-x1)*m)
		 */
		
		PBDLine vline = e.line;
		
		PBDLine[] line = this.getEpsilonNeiborhood(vline, eps);
		...
		double[] intersectionWithDiscOne;
		double[] intersectionWithDiscTwo;
		...
		try {
			...
			System.out.println(startIntervalOnCurve + " " + endIntervalOnCurve + " " + startIntervalOnEdge + " " + endIntervalOnEdge);
	...
	public PBDVertex getVertex(double t) {
		return new PBDVertex(this.p1.x + this.xdiff * t, this.p1.y + this.ydiff * t);
	}
	
	public PBDLine[] getEpsilonNeiborhood(PBDLine vline, double eps) {
		
		PBDLine[] line = new PBDLine[2];
		double dx, dy;
		...
		line[0] = new PBDLine(new PBDVertex(vline.p1.x - dx, vline.p1.y - dy),
				new PBDVertex(vline.p2.x - dx, vline.p2.y - dy));
		line[1] = new PBDLine(new PBDVertex(vline.p1.x + dx, vline.p1.y + dy),
				new PBDVertex(vline.p2.x + dx, vline.p2.y + dy));
	...
	public double[] getTParallel(PBDLine line, double eps) {
		double[] t = new double[2];
		double newm;
		double x1, y1, x2, y2;
	...
	public double distance(PBDVertex p) {
