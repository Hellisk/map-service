...
import static edu.uq.dke.mapupdate.Main.AHMED_EPSILON;
...
public class AhmedTraceMerge2012 {
    ...
    private boolean isWhiteInterval(Edge edge, List<Vertex> pose,
     ...
     */
    private void setEndPointsOnEdge(Edge edge, int startIndex, int endIndex,
     ...
     * pose.
     */
    private void computeNextInterval(Edge edge, List<Vertex> pose, int newstart,
     ...
     */
    private void updateMap(List<Vertex> constructedMap,
     ...
     */
    private void edgeSplit(List<Vertex> constructedMap,
    ...
    private void commitEdgeSplits(List<Edge> edges, Map<String, Integer> map,
                                  List<Vertex> graph) {
        ...
        for (int i = 0; i < edges.get(1).getEdgeSplitPositions().size(); i++) {
            ...
            edge.addSplit(newPosition, edges.get(1).getEdgeSplitVertices().get(i));
        }
        ...
        for (int i = 0; i < v1.getDegree(); i++) {
            if (v1.getAdjacentElementAt(i) == index2) {
                ...
                graph.get(edgeVertexSplits.get(0))
        ...
        for (int i = 0; i < v2.getDegree(); i++) {
            if (v2.getAdjacentElementAt(i) == index1) {
                ...
                graph.get(edgeVertexSplits.get(splitSize - 1))
        ...
        if (!(updateV1 && updateV2)) {
            logger.log(Level.SEVERE, "inconsistent graph: (" + splitSize + ")"
                    + index1 + " " + index2 + " " + v1.getAdjacencyList().toString() + " " + v2.getAdjacencyList().toString());
        }
    }
    ...
    private void commitEdgeSplitsAll(List<Vertex> constructedMap,
    ...
    private void addToGraph(List<Vertex> constructedMap, List<Vertex> pose,
    ...
    private void updateSiblingHashMap(Map<String, ArrayList<Edge>> siblingMap,
                                      Edge edge) {
    ...
    private void mapConstruction(List<Vertex> constructedMap, List<Edge> edges,
        ...
        for (Edge edge1 : edges) {
            this.computeNextInterval(edge1, pose, 1, eps, altEps);
            if (!edge1.getDone()) {
                pq.add(edge1);
            }
        }
    ...
    private List<PoseFile> readAllFiles(File folder) {
        List<PoseFile> poseFiles = new ArrayList<PoseFile>();
        for (File file : folder.listFiles()) {
            poseFiles.add(PoseFile.readFile(file));
        }
        return poseFiles;
    }
    ...
    private List<Vertex> constructMapMain(List<PoseFile> poseFiles, double eps,
                                          double altEps) {
    ...
    public static RoadNetworkGraph AhmedTraceMerge(String cityName, String inputPath) throws JDOMException, IOException {
        ...
        List<Vertex> constructedMap = mapConstruction.constructMapMain(
                mapConstruction.readAllFiles(new File(inputPath)),
                AHMED_EPSILON, 4.0);
