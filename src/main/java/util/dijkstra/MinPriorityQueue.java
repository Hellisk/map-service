...
/**
 * Minimal heap used in Dijkstra algorithm.
 */
...
//        double distance = root.distance;
	...
	public boolean decreaseKey(int index, double distance) {
		if (insertedNodes.containsKey(index)) {   // the coming key already exists
            ...
            if (distance < queryNode.distance) {  // the updateGoh is valid
				...
				while (queryNode.distance < queryNode.parent.distance || (queryNode.distance == queryNode.parent.distance
						&& queryNode.arrayIndex < queryNode.parent.arrayIndex)) {
	...
	private void insert(Item newPoint) {
		if (this.heapSize != 0) {
			...
			while (newPoint.distance < newPoint.parent.distance || (newPoint.distance == newPoint.parent.distance
					&& newPoint.arrayIndex < newPoint.parent.arrayIndex)) {
