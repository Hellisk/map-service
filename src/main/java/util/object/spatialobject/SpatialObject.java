...
import util.exceptions.SpatialRelationException;
 ...
 */
public interface SpatialObject extends Cloneable {
	...
	/**
	 * Retrieve the distance function for the current coordination system which is used for all distance-based calculation.
	 *
	 * @return The distance function used in the current object.
	 */
	@NonNull
	...
	default Rect mbr() {
		...
		if (coordList == null || coordList.isEmpty()) {
			throw new SpatialObjectConstructionException("Cannot calculate MBR. Object is empty or null.");
		}
		...
		return new Rect(minX, minY, maxX, maxY, getDistanceFunction());
	}
	
	/**
	 * Check whether this spatial object is a closed object.
	 *
	 * @return True if the object is composed by edges (or a circle) and its first and final edge points are the same.
	 */
	boolean isClosed();
	 ...
	 * <p>
	 * For ComplexSpatialObjects this method verifies if every coordinate of the two objects are the same in the exact order they are
	...
	/**
	 * Compare this spatial object using the given comparator. The result of this method is the same as comparator.compare(this, obj).
	 *
	 * @param obj        The spatial object to compare.
	...
	/**
	 * Cast this spatial object to an equivalent JavaTopologicalSuit (JTS) geometry object.
	 *
	 ...
	 * <br> The boundary and interior of the second geometry are not allowed to intersect the exterior of the first geometry and the
	 * geometries may not be equal.
	 * <br> The interiors of both geometries must intersect and that the interior and boundary of the secondary (geometry b) must not
	 * intersect the exterior of the primary (geometry a).
	 *
	 * @param obj The spatial object to check.
	 * @return Returns TRUE if the given spatial object 'obj' is completely contained by this spatial object.
	...
	default boolean contains(SpatialObject obj) throws SpatialRelationException {
		...
		try {
			...
			throw new SpatialRelationException("'Contains' operation not supported for these spatial objects.");
		}
	}
	 ...
	 * <br> The boundary and interior of the first geometry are not allowed to intersect the exterior of the second geometry and the
	 * first geometry may not equal the second geometry.
	 * <br> The interiors of both geometries must intersect and that the interior and boundary of the primary geometry (geometry a) must
	 * not intersect the exterior of the secondary (geometry b).
	 *
	 * @param obj The spatial object to check.
	 * @return Returns TRUE if this spatial object is completely within the given spatial object 'obj'.
	 * @throws SpatialRelationException If the operation is not supported by these spatial objects.
	 */
	default boolean within(SpatialObject obj) throws SpatialRelationException {
		...
		try {
			...
			throw new SpatialRelationException("'Within' operation not supported for these spatial objects.");
		}
	}
	 ...
	 * Check whether the given spatial object is completely contained by this spatial object.
	 * <p>
	 * ''a'' is covered by ''b'' (extends Within) if every point of ''a'' is a point of ''b'', and the interiors of the two geometries
	...
	default boolean coveredBy(SpatialObject obj) throws SpatialRelationException {
		...
		try {
			...
			throw new SpatialRelationException("'CoveredBy' operation not supported for these spatial objects.");
		}
	}
	 ...
	 * <p>
	 * ''a'' covers ''b'' if no points of ''b'' lie in the exterior of ''a'', or every  point of ''b'' is a point of (the interior or
	 * boundary of) ''a''.
	 *
	 * @param obj The spatial object to check.
	...
	default boolean covers(SpatialObject obj) throws SpatialRelationException {
		...
		try {
			...
			throw new SpatialRelationException("'Covers' operation not supported for these spatial objects.");
		}
	}
	 ...
	 * <p>
	 * Returns TRUE if the intersection results in a geometry whose dimension is one less than the maximum dimension of the two source
	 * geometries, and the intersection set is interior to both source geometries.
	 * <br> The interiors must intersect and at least the interior of the primary (geometry a) must intersect the exterior of the
	...
	default boolean crosses(SpatialObject obj) throws SpatialRelationException {
		...
		try {
			...
			throw new SpatialRelationException("'Crosses' operation not supported for these spatial objects.");
		}
	}
	 ...
	 * <br> The intersects predicate returns TRUE if the boundary of the first geometry intersects the boundary of the second geometry.
	 * <br> The intersects predicate returns TRUE if the boundary of the first geometry intersects the interior of the second.
	 ...
	 * <p>
	 * Compares two geometries of the same dimension and returns TRUE if their intersection set results in a geometry different from
	 * both, but of the same dimension.
	 * <br> Overlap returns TRUE only for geometries of the same dimension, and only when their intersection set results in a geometry of
	 * the same dimension. In other words, if the intersection of two polygons results in polygon, then overlap returns TRUE.
	 * <br> The overlap predicate returns TRUE if the interior of both geometries intersects the others interior and exterior.
	 *
	 * @param obj The spatial object to check.
	...
	default boolean overlaps(SpatialObject obj) throws SpatialRelationException {
		...
		try {
			...
			throw new SpatialRelationException("'Overlaps' operation not supported for these spatial objects.");
		}
	}
	
	/**
	 * Clone the semantic attributes (super-class attributes) of this spatial object into the given spatial object (auxiliary clone method).
	 *
	 * @param obj The spatial object to receive the cloned attributes.
	 */
	default void cloneTo(SpatialObject obj) {
		if (obj == null) {
			throw new NullPointerException("Spatial object to receive cloned attributes cannot be null.");
		}
		if (this.getID() != null) {
