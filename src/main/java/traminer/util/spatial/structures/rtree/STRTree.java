...
public class STRTree<T extends SpatialObject> implements SpatialDataStructure<T> {
     ...
     */
    private RTreeModel rtreeModel = null;
    /**
    ...
    public STRTree(List<XYObject<T>> objectsList, int nodesMaxCapacity) {
        if (objectsList == null || objectsList.isEmpty()) {
            throw new IllegalArgumentException("Object list for STR "
                    + "packing cannot be empty.");
        }
     ...
     * @param nodesMaxCapacity The maximum number of objects
     *                         in each node/partition of this tree.
     * @param nodesFillRate    The nodes initial filling rate ]0.0,..,1.0]. The Tree will
     *                         be initially constructed with a maximum of (nodesMaxCapacity * nodesFillRate)
     *                         objects in each leaf node.
     */
    public STRTree(List<XYObject<T>> objectsList, int nodesMaxCapacity, double nodesFillRate) {
        ...
        if (nodesFillRate <= 0 || nodesFillRate > 1.0) {
            throw new IllegalArgumentException("Nodes fill rate "
    ...
    private void build(List<XYObject<T>> objectsList) {
        ...
        for (int i = 0; i < verticalSlices.size(); i++) {
            Slice slice = verticalSlices.get(i);
            minx = slice.minX;
            maxx = slice.maxX;
            // sort the objects in every slice by y coordinate
            Collections.sort(slice, XYObject.Y_COMPARATOR);
            // pack the point into vertical slices
            ...
            for (int j = 0; j < slice.size(); ) {
                ...
                STRNode<T> node = new STRNode<T>("" + countId++, minx, miny, maxx, maxy);
     ...
     * (leaf node) of this tree.
     * <br> Note that adding objects to a STR tree after the tree has been
     * constructed will have no effect on its partitioning model. This method
     * will simply insert the object into an existing partition only if the
     * number of objects in the partition is smaller than its maximum capacity.
     *
     * @return True if the object was successfully inserted in the tree. False
     * if this tree does not contain the object in its boundaries, or if the
     * node to insert this object is already full.
     */
    @Override
     ...
     * <br> Note that removing objects from a STR tree after the tree has been
     * constructed will have no effect on its partitioning model. This method
     * will simply remove the object from an existing partition (leaf) containing
     * this object.
     *
     * @return True if the object was successfully removed.
     * False if this spatial data structure does not contain
     * the given object to remove.
     */
    @Override
    ...
    /**
     * Auxiliary class representing a slice/partition in the STR-Tree.
     * Slices are vertical partitions along the X axis.
     */
