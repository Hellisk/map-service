...
public class LineTM {
	...
	private VertexTM p1; // first endpoint
	private VertexTM p2; // second endpoint
	...
	private double theta; // smallest angle between x-axis and this LineTM object
	// private static final FormattingLogger logger = FormattingLogger.getLogger(LineTM.class);
	
	/**
	 * Constructor for a LineTM object.
	 *
	 * @param p1 first endpoint
	 * @param p2 second endpoint
	 */
	public LineTM(VertexTM p1, VertexTM p2, DistanceFunction df) {
		this.p1 = p1;
		this.p2 = p2;
		this.distFunc = df;
		...
		if (this.xdiff != 0.0) {
			...
			VertexTM vector = new VertexTM(p2.getX() - p1.getX(), p2.getY() - p1.getY(), distFunc);
			double angle1 = VertexTM.dotProd(vector, new VertexTM(1.0, 0.0, distFunc)) / vector.norm();
	 ...
	 * @return LineTM[2] containing two lines
	 */
	private static LineTM[] getEpsilonNeighborhood(LineTM vline, double eps) {
		// compute the equations of boundaries of eps-region around the line
		LineTM[] lines = new LineTM[2];
		...
		lines[0] = new LineTM(
				new VertexTM(vline.getP1().getX() - dx, vline.getP1().getY() - dy, vline.getDistanceFunction()),
				new VertexTM(vline.getP2().getX() - dx, vline.getP2().getY() - dy, vline.getDistanceFunction()), vline.getDistanceFunction());
		lines[1] = new LineTM(
				new VertexTM(vline.getP1().getX() + dx, vline.getP1().getY() + dy, vline.getDistanceFunction()),
				new VertexTM(vline.getP2().getX() + dx, vline.getP2().getY() + dy, vline.getDistanceFunction()), vline.getDistanceFunction());
	...
	public VertexTM getP1() {
		return this.p1;
	}
	
	public VertexTM getP2() {
		return this.p2;
	}
	...
	public String toString() {
		...
		return String.format("[ %f %f; %f %f]", this.p1.getX(), this.p1.getY(), this.p2.getX(), this.p2.getY());
	}
	 ...
	 */
	private double[] pIntersection(VertexTM p, double eps, boolean precisionCompromise) {
		
		/*
		 * LineTM 1: x1+(x2-x1)*t = x; x1+xdiff*t = x y1+(y2-y1)*t = y; y1+ydiff*t = y
		 *
		...
		if (determinant >= 0) {
			...
			double newC = (p1.getX() - p.getX()) * (p1.getX() - p.getX()) + (p1.getY() - p.getY()) * (p1.getY() - p.getY()) - newEps * newEps;
	...
	/**
	 * Returns intersection points of this line and two lines in LineTM[] (neighborhood). For detail see
	 * {@link package-info}
	 *
	 ...
	 */
	private double[] getIntervals(LineTM[] lines) {
	 ...
	 */
	private void setIntervalOnEdge(EdgeTM e, double eps, double cIntervalStart, double cIntervalEnd, double vstart, double vend) {
		...
		if (vstart == -1) {
			...
			if (in1 == null) {
				//logger.log(Level.SEVERE, "Problem computing LineTM intersection: in1.");
		...
		if (vend == -1) {
			...
			if (in2 == null) {
				//logger.log(Level.SEVERE, "Problem computing LineTM intersection: in2.");
				throw new RuntimeException();
			}
	...
	boolean pIntersection(EdgeTM e, double eps) {
		
		/*
		 * LineTM 1: x1+(x2-x1)*t = x y1+(y2-y1)*t = y
		 *
		 * LineTM 2: y = mx + c
		 *
		...
		LineTM vline = e.getLine();
		
		LineTM[] lines = LineTM.getEpsilonNeighborhood(vline, eps);
		double cIntervalStart;
		double cIntervalEnd;
		double neighborhoodStart;
		double neighborhoodEnd;
		...
		if (Math.abs(this.theta - vline.getTheta()) == 0 || Math.abs(this.theta - vline.getTheta()) == 180.0) {// For parallel lines
	...
	/**
	 * Get a VertexTM on this line with parameter t.
	 *
	 * @param t the parameter
	 * @return a vertex on this line with parameter t.
	 */
	VertexTM getVertex(double t) {
		return new VertexTM(this.p1.getX() + this.xdiff * t, this.p1.getY() + this.ydiff * t, this.distFunc);
	}
	 ...
	 */
	public double distance(VertexTM p) {
		Point point = new Point(p.getX(), p.getY(), this.distFunc);
		Segment segment = new Segment(this.p1.getX(), this.p1.getY(), this.p2.getX(), this.p2.getY(), distFunc);
		return distFunc.distance(point, segment);
	}
	...
	private double[] getTParallel(LineTM line, double eps) {
		double[] t = new double[2];
		double newm;
		double x1, y1, x2, y2;
		...
		if (distFunc.pointToPointDistance(line.getP1().getX(), line.getP1().getY(), x1, y1) > eps) {
			return null;
		}
	...
	/**
	 * Computes t value on this line for VertexTM v, t = 0 at p1 and t = 1 at p2.
	 *
	 * @return a double value.
	 */
	public double tValueOnLine(VertexTM v) {
		if (this.xdiff == 0) {
	 ...
	 */
	public boolean onLine(VertexTM v) {
