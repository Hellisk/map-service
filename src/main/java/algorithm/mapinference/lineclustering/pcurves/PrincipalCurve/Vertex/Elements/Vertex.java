...
abstract public class Vertex extends Vektor2D {
	...
	final protected EndVertexOfTwo MaintainEndVertex(Vertex neighbor) {
		...
		} catch (algorithm.mapinference.lineclustering.pcurves.PrincipalCurve.Vertex.TwoOppositeEdgeIndexesException e1) {
			try {
				algorithm.mapinference.lineclustering.pcurves.PrincipalCurve.Vertex.YVertex realNeighbor =
						(algorithm.mapinference.lineclustering.pcurves.PrincipalCurve.Vertex.YVertex) neighbor;
				return new EndVertexOfY(this, realNeighbor.GetEdgeIndex3(), realNeighbor.GetEdgeIndex1(), realNeighbor
						.GetEdgeIndex2());
			} catch (ClassCastException e2) {
				algorithm.mapinference.lineclustering.pcurves.PrincipalCurve.Vertex.TVertex realNeighbor =
						(algorithm.mapinference.lineclustering.pcurves.PrincipalCurve.Vertex.TVertex) neighbor;
				return new EndVertexOfTwo(this, realNeighbor.GetEdgeIndex(this).edgeIndex);
			}
		}
	}
	...
	// LineTM oldalfelezoMeroleges = LineObject.GetNormalAt(midPoint,difference,new Vektor2D(0,0));
	...
	// }
	// else {
	// LineTM tangent = LineObject.GetNormalAt(this,center.Sub(this),new Vektor2D(0,0));
