...
public class MapGenerator {
	...
	public static RoadNetworkGraph geoErrorMapGenerator(RoadNetworkGraph originalMap, double percentage, double radius) {
		...
		for (Integer index : selectedIndex) {
			...
			double newX = originalNode.lon() + distFunc.getCoordinateOffsetX(distance * Math.cos(angle), originalNode.lat());
			double newY = originalNode.lat() + distFunc.getCoordinateOffsetY(distance * Math.sin(angle), originalNode.lon());
			...
			for (RoadWay way : originalNode.getInComingWayList()) {
				...
				for (int i = 1; i < nodes.size() - 1; i++) {
					...
					double newMiniX = node.lon() + ratio * (newX - originalX);
					double newMiniY = node.lat() + ratio * (newY - originalY);
...
//						count++;
	 ...
	 * @return The returning error map.
	 */
	public static RoadNetworkGraph roadShapeErrorMapGenerator(RoadNetworkGraph originalMap, double percentage,
															  double distance, boolean isCompleteRandom) {
		DistanceFunction distFunc = originalMap.getDistanceFunction();
		List<RoadWay> candidateWayList;
		if (isCompleteRandom)
			candidateWayList = completeRandomWayList(originalMap, percentage);
		...
		for (RoadWay way : candidateWayList) {
			...
			if (id2processedWayMap.containsKey(reverseWayID)) {    // the reverse direction road should have the same shape with the
				...
				for (int i = originalNodeList.size() - 2; i > 0; i--) {
					String reverseNodeID = originalNodeList.get(i).getID().contains("-") ? originalNodeList.get(i).getID().substring(0,
							originalNodeList.get(i).getID().length() - 1) : originalNodeList.get(i).getID() + "-";
			...
			for (int i = 0; i < way.getNodes().size() - 1; i++) {
				...
				int idIndex = 0;
				double length = distFunc.distance(currNode.toPoint(), nextNode.toPoint());
				while (length > distance) {    // insert new nodes.
					double angle = distFunc.getHeading(currNode.lon(), currNode.lat(), nextNode.lon(), nextNode.lat());
					if (direction)
						angle = angle + Math.PI / 2;
					else
						angle = angle - Math.PI / 2;
					double newLon = (currNode.lon() + nextNode.lon()) / 2 + distFunc.getCoordinateOffsetX(Math.cos(angle) * distance,
							currNode.lat());
					double newLat = (currNode.lat() + nextNode.lat()) / 2 + distFunc.getCoordinateOffsetX(Math.sin(angle) * distance,
							currNode.lon());
					RoadNode newNode = new RoadNode(currNode.getID() + "+" + idIndex, newLon, newLat, distFunc);
			...
			if ((currNode.getDegree() == 4 && currNode.getInComingDegree() == currNode.getOutGoingDegree()) || currNode.getDegree() > 4) {
				...
				while (newNode.getDegree() < currNode.getDegree()) {
					if (currNode.getInComingDegree() > currNode.getOutGoingDegree()) {    // select a road from in-coming roads.
						...
						List<RoadNode> wayNodeList = new ArrayList<>();
						wayNodeList.add(newNode);
						wayNodeList.addAll(currWay.getNodes());
	...
	private static List<RoadWay> weightedRandomWayList(RoadNetworkGraph originalMap, double percentage) {
		...
		while (selectIDSet.size() < itemRange.length * percentage / 100.0) {    // keep finding the next road
			...
			while (value > itemRange[index] || value < itemRange[index - 1]) {
